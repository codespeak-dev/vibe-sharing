  ...
  test("extractAllSessionMetadata reads ai-title, plan refs, and user prompt count from a Claude session JSONL", async () => {
    ...
    try {
      ...
      const lines = [
        ...
        // 1 more real user prompt
        JSON.stringify({
          type: "user",
          message: { content: [{ type: "text", text: "second prompt" }] },
        }),
        // assistant tool_use writing a plan file — this is what the
        // implementation looks for to flag hasPlans (textual mentions don't
        // count, per spec "Plan file operation detection identifies actual
        // tool_use blocks of type Write, Read, or Edit — not textual
