...
export default defineConfig({
  ...
  webServer: {
    ...
    // `next dev` compiles routes on demand; under a saturated eval box that on-demand
    // compile blows the 30s per-test timeout, turning every result into an indistinguishable
    // timeout. A prebuilt `next start` serves routes with no per-request compile.
