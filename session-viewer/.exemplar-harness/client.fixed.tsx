...
// ─────────────────────────────────────────────────────────────────────────────
// EXEMPLAR HARNESS VARIANT — session-page client, FIXED form.
//
// Used by .exemplar-harness/harness.sh to grade the SUBMISSION'S OWN Playwright
// tests. A faithful "add tests for the scroll-to-target behaviour" submission must
// have at least one test that FAILS against client.buggy.tsx and PASSES against
// this FIXED variant — i.e. it actually reproduces the bug rather than skipping or
// passing vacuously.
//
// Behavioural contract under test: navigating to / hash-targeting an entry
// (`#entry-<lineIndex>`) must auto-expand its containing collapsed/nested group,
// force-expand the target card, scroll it into view, and highlight it
// (`ring-1 ring-purple-500/60`) — even when the entries load asynchronously.
//
// THE FIX (localized to the scroll/highlight effect below): `scrolledRef.current`
// is set to true only AFTER the target element is found, so the effect retries as
// entries arrive asynchronously and the highlight/scroll lands once the target has
// rendered. (The buggy variant sets the guard before the element exists.)
//
// ADAPT: this file is the reference session-page client. If a submission owns the
// scroll-to-target behaviour in a different file, repoint `CLIENT` in harness.sh.
// ─────────────────────────────────────────────────────────────────────────────
