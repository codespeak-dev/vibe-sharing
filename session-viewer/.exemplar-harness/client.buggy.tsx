...
// ─────────────────────────────────────────────────────────────────────────────
// EXEMPLAR HARNESS VARIANT — session-page client, BUGGY form.
//
...
// have at least one test that FAILS against this BUGGY variant and PASSES against
// client.fixed.tsx — i.e. it actually reproduces the bug rather than skipping or
// passing vacuously.
//
...
//
// THE BUG (localized to the scroll/highlight effect below): `scrolledRef.current`
// is set to true BEFORE the target element exists, so when entries arrive
// asynchronously the effect's retry is blocked and the highlight/scroll never
// lands. (The fixed variant sets the guard only after the element is found.)
//
