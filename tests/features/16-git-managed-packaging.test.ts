...
describe("Feature: Git-Managed Project Packaging", () => {
  ...
  test("Archive sessions directory replicates local .claude folder structure (sessions/.claude/projects/<encoded>/...)", async () => {
    const proj = await makeTmpProject();
    try {
      await gitInit(proj.root);
      await writeFile(proj.root, "README.md", "# project\n");
      ...
      // Build a fake archive root that mimics ~/.claude with one project
      // session file under projects/<encoded>/.
      const fakeClaudeRoot = await fs.mkdtemp(
        path.join(require("node:os").tmpdir(), "fake-claude-"),
      );
      const sessionRel = path.join("projects", "-Users-test-proj", "abc.jsonl");
      const sessionAbs = path.join(fakeClaudeRoot, sessionRel);
      await fs.mkdir(path.dirname(sessionAbs), { recursive: true });
      await fs.writeFile(sessionAbs, '{"type":"user"}\n');

      const fakeProvider = {
        name: "Claude Code",
        slug: "claude-code",
        async detect() { return true; },
        async discoverProjects() { return new Map(); },
        async findSessions() { return []; },
        async getSessionFiles() { return []; },
        async getProviderFiles() { return [sessionAbs]; },
        getArchiveRoot() { return fakeClaudeRoot; },
      };
      ...
      const result = await createArchive({
        ...
        selectedSessionIds: new Set(["abc"]),
        manifest,
      });

      try {
        // List zip entries via the system `unzip -Z1` (no library dep).
        const { execFileSync } = await import("node:child_process");
        const listing = execFileSync("unzip", ["-Z1", result.zipPath], {
          encoding: "utf8",
        });
        const entries = listing.split("\n").map((s) => s.trim()).filter(Boolean);

        // The session file should appear at sessions/<archiveRootBasename>/<rel>
        // mirroring the local layout — e.g. sessions/.claude/projects/.../abc.jsonl.
        const expected = `sessions/${path.basename(fakeClaudeRoot)}/${sessionRel.replace(/\\/g, "/")}`;
        expect(entries).toContain(expected);
        if (state.bundlePath) cleanupBundle(state.bundlePath);
      } finally {
        cleanupArchive(result.zipPath);
        await fs.rm(fakeClaudeRoot, { recursive: true, force: true }).catch(() => {});
      }
    } finally {
      await proj.cleanup();
    }
